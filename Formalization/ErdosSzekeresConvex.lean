...
(no three points collinear) contains the vertices of a strictly convex $k$-gon.
...
2. `Formalization.ErdosSzekeresConvex.Sorting`: 2D plane rotations, lexicographical order on $\mathbb{R}^2$,
   the sorting theorem for finite sets with distinct $x$-coordinates, and the finite slope avoidance theorem.
...
4. `Formalization.ErdosSzekeresConvex.ConvexPolygon`: Transitivity of orientations, 4-point determinant identities,
   strict convex polygon formation from cups and caps, and rotation invariance of convex polygons.
...
/-- **Main Theorem: Erdős–Szekeres Convex Polygon Theorem (1935).**
    Every set of at least `erdosSzekeresBound k` points in general position
...
theorem erdos_szekeres_convex_polygon (k : ℕ) (hk : 3 ≤ k)
  ...
  obtain ⟨c, s, h_unit, h_dist⟩ := exists_rotation_hasDistinctX S
  have h_gen_rot := inGeneralPosition_rotate2D S c s h_unit h_gen
  have h_card_rot : erdosSzekeresBound k ≤ (S.image (rotate2D c s)).card := by
    rwa [Finset.card_image_of_injective S (rotate2D_injective c s h_unit)]
  have h_poly_rot : FormsConvexPolygon (S.image (rotate2D c s)) k := by
    rcases cup_cap_lemma k k hk hk (S.image (rotate2D c s)) h_dist (by rwa [show k + k = 2 * k by omega]) h_gen_rot with ⟨C, hC, hC_sub⟩ | ⟨C, hC, hC_sub⟩
    · exact formsConvexPolygon_of_isCup _ C k hk hC hC_sub
    · exact formsConvexPolygon_of_isCap _ C k hk hC hC_sub
  exact formsConvexPolygon_of_rotate2D S k c s h_unit h_poly_rot

/-- For k = 3, every set of at least 3 points in general position
    contains a convex triangle. Exact evaluation: ES(3) = 3. -/
theorem erdos_szekeres_triangle (S : Finset Point2D)
    (h_card : 3 ≤ S.card)
    (h_gen : InGeneralPosition S) :
    FormsConvexPolygon S 3 :=
  erdos_szekeres_convex_polygon 3 (by omega) S h_card h_gen

/-- For k = 4, every set of at least 7 points in general position
...
theorem erdos_szekeres_four_points (S : Finset Point2D)
  ...
  erdos_szekeres_convex_polygon 4 (by omega) S h_card h_gen

/-- **Esther Klein's Theorem (1935 / Klein's Problem 1835):**
    Every set of at least `erdosSzekeresBound 4` (= 7) points in general position
