...
2. **Group-Theoretic & Algebraic Extensions (`Formalization.SchursTheorem.Group`)**:
   - `SchursTheorem.group_schurs_theorem`: Multiplicative Schur theorem in arbitrary groups $(G, \cdot)$
     via difference/Cayley graph colorings ($x \cdot y = z$).
   - `SchursTheorem.addCommGroup_schurs_theorem`: Additive Schur theorem in arbitrary abelian groups $(A, +)$.
...
3. **Quantitative Supersaturation & Counting (`Formalization.SchursTheorem.Quantitative`)**:
   ...
   - `SchursTheorem.card_monoSchurTriples_one`: Exact quadratic formula $\frac{(N-1)N}{2}$ for $r = 1$.
   - `SchursTheorem.card_monoTriangles_ge_k`: Disjoint block embedding proving at least $k$ graph triangles.
   - `SchursTheorem.supersaturation_bound`: Positive supersaturation lower bound $k \le N \cdot |\text{monoSchurTriples } \chi N|$.
...
4. **Single-Equation Rado Partition Regularity (`Formalization.SchursTheorem.Rado`)**:
   - `SchursTheorem.rado_zero_sum_partition_regular`: Partition regularity of zero-sum linear equations $\sum c_i = 0$.
   - `SchursTheorem.schur_is_rado_regular`: Schur's equation $(1, 1, -1)$ as a Rado partition regular equation.
