...
theorem euler_planar_nat : ∀ P : PlanarMap, P.vertexCount + P.faceCount = P.edgeCount + 2
  | vertex => rfl
  | addPendant p => by have := euler_planar_nat p; dsimp [vertexCount, faceCount, edgeCount]; omega
  | addFaceEdge p => by have := euler_planar_nat p; dsimp [vertexCount, faceCount, edgeCount]; omega
...
theorem euler_char_eq_two (P : PlanarMap) : P.eulerChar = 2 := by
  have := P.euler_planar_nat; dsimp [eulerChar]; omega
...
lemma vertexCount_pos : ∀ P : PlanarMap, 0 < P.vertexCount
  | vertex => by decide
  | addPendant p => by have := p.vertexCount_pos; dsimp [vertexCount]; omega
  | addFaceEdge p => p.vertexCount_pos

/-- Every planar map has at least one face. -/
lemma faceCount_pos : ∀ P : PlanarMap, 0 < P.faceCount
  | vertex => by decide
  | addPendant p => p.faceCount_pos
  | addFaceEdge p => by have := p.faceCount_pos; dsimp [faceCount]; omega

/-- The edge count is bounded by vertices and faces. -/
lemma edgeCount_le (P : PlanarMap) : P.edgeCount + 1 ≤ P.vertexCount + P.faceCount := by
  have := euler_planar_nat P; omega
...
lemma tree_faceCount_eq_one : ∀ P : PlanarMap, P.isPendantTree → P.faceCount = 1
  | vertex, _ => rfl
  | addPendant p, h => tree_faceCount_eq_one p h
...
lemma tree_edgeCount (P : PlanarMap) (h : P.isPendantTree) : P.edgeCount + 1 = P.vertexCount := by
  have hF := tree_faceCount_eq_one P h; have hE := euler_planar_nat P; omega

/-- Total edges decompose into pendant edges and non-tree edges. -/
lemma edgeCount_eq_pendant_add_nonTree : ∀ P : PlanarMap, P.edgeCount = P.pendantEdgeCount + P.nonTreeEdgeCount
  | vertex => rfl
  | addPendant p => by have := edgeCount_eq_pendant_add_nonTree p; dsimp [edgeCount, pendantEdgeCount, nonTreeEdgeCount]; omega
  | addFaceEdge p => by have := edgeCount_eq_pendant_add_nonTree p; dsimp [edgeCount, pendantEdgeCount, nonTreeEdgeCount]; omega

/-- The vertex count is determined by pendant edges: V = E_pendant + 1. -/
lemma vertexCount_eq_pendant_add_one : ∀ P : PlanarMap, P.vertexCount = P.pendantEdgeCount + 1
  | vertex => rfl
  | addPendant p => by have := vertexCount_eq_pendant_add_one p; dsimp [vertexCount, pendantEdgeCount]; omega
  | addFaceEdge p => vertexCount_eq_pendant_add_one p

/-- The face count is determined by non-tree edges: F = E_nonTree + 1. -/
lemma faceCount_eq_nonTree_add_one : ∀ P : PlanarMap, P.faceCount = P.nonTreeEdgeCount + 1
  | vertex => rfl
  | addPendant p => faceCount_eq_nonTree_add_one p
  | addFaceEdge p => by have := faceCount_eq_nonTree_add_one p; dsimp [faceCount, nonTreeEdgeCount]; omega
...
lemma edgeFinset_card_le_of_le (G T : SimpleGraph V) [DecidableRel G.Adj] [DecidableRel T.Adj]
    (hle : T ≤ G) : T.edgeFinset.card ≤ G.edgeFinset.card :=
  Finset.card_le_card (fun _ he => SimpleGraph.mem_edgeFinset.mpr <| SimpleGraph.edgeSet_mono hle <| SimpleGraph.mem_edgeFinset.mp he)
...
theorem euler_connected_graph (G : SimpleGraph V) [DecidableRel G.Adj]
  ...
  have := hT.card_edgeFinset; have := edgeFinset_card_le_of_le G T hle; dsimp [spanningTreeFaceCount]; omega
...
lemma spanningTreeFaceCount_pos (G : SimpleGraph V) [DecidableRel G.Adj]
  ...
  dsimp [spanningTreeFaceCount]; omega
...
theorem connected_edge_ge_vert_sub_one (G : SimpleGraph V) [DecidableRel G.Adj]
  ...
  let _ : DecidableRel T.Adj := Classical.decRel T.Adj
  have := hT.card_edgeFinset; have := edgeFinset_card_le_of_le G T hle; omega
...
theorem euler_connected_graph_exists (G : SimpleGraph V) [DecidableRel G.Adj]
  ...
  obtain ⟨T, hle, hT⟩ := hconn.exists_isTree_le
  let _ : DecidableRel T.Adj := Classical.decRel T.Adj
  exact ⟨T, inferInstance, hT, hle, euler_connected_graph G T hT hle⟩
...
theorem euler_polyhedron_formula (P : PlanarMap) : P.eulerChar = 2 := P.euler_char_eq_two
...
theorem non_planarity_k5 (F : ℕ) (h_euler : (5 : ℤ) - 10 + F = 2) (h_face_deg : 3 * F ≤ 2 * 10) : False := by
...
theorem non_planarity_k33 (F : ℕ) (h_euler : (6 : ℤ) - 9 + F = 2) (h_face_deg : 4 * F ≤ 2 * 9) : False := by
