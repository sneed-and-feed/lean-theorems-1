...
- `erdos_ko_rado_uniqueness_one`: The exact EKR equality/uniqueness case for one-element sets.
- `hilton_milner_stability_two`: The full Hilton--Milner bound for two-element sets, via an
  exact classification of every non-star intersecting family as a triangle.
...
theorem one_uniform_intersecting_eq_singleton
    ...
    (h_inter : ∀ A ∈ F, ∀ B ∈ F, ¬ Disjoint A B) :
    ∃ x : α, F = {{x}} := by
  obtain ⟨A, hAF⟩ := hF_nonempty
  obtain ⟨x, rfl⟩ := card_eq_one.mp (hF_one A hAF)
  refine ⟨x, Finset.ext fun B => ?_⟩
  simp only [mem_singleton]
  constructor
  · intro hBF
    obtain ⟨y, rfl⟩ := card_eq_one.mp (hF_one B hBF)
    have hxy := h_inter {x} hAF {y} hBF
...
theorem erdos_ko_rado_uniqueness_one {n : ℕ}
    (F : Finset (Finset α))
    (hF_one : ∀ A ∈ F, A.card = 1)
    (h_inter : ∀ A ∈ F, ∀ B ∈ F, ¬ Disjoint A B)
    (h_max : F.card = Nat.choose (n - 1) (1 - 1)) :
    IsStarFamily F := by
  have hFcard : F.card = 1 := by simpa using h_max
  have hF_nonempty : F.Nonempty := card_pos.mp (by omega)
  obtain ⟨x, hF⟩ := one_uniform_intersecting_eq_singleton F hF_nonempty hF_one h_inter
  refine ⟨x, ?_⟩
  rw [hF]
  simp
...
/-- For `4 < n`, the Hilton--Milner expression at `k = 2` is exactly three. -/
lemma hiltonMilnerBound_two {n : ℕ} (h4n : 4 < n) :
    hiltonMilnerBound n 2 = 3 := by
  rw [hiltonMilnerBound]
  simp only [Nat.reduceSubDiff, Nat.choose_one_right]
  omega
...
private lemma exists_pair_eq_of_mem_of_card_eq_two {B : Finset α} {y : α}
    (hyB : y ∈ B) (hB : B.card = 2) :
    ∃ z, y ≠ z ∧ B = {y, z} := by
  have herase : (B.erase y).card = 1 := by
    rw [card_erase_of_mem hyB, hB]
  obtain ⟨z, hz⟩ := card_eq_one.mp herase
  refine ⟨z, ?_, ?_⟩
  · intro hyz
    subst z
    have : y ∈ B.erase y := by simp [hz]
...
theorem two_uniform_intersecting_not_star_eq_triangle
    [Nonempty α] (F : Finset (Finset α))
    (hF_two : ∀ A ∈ F, A.card = 2)
    (h_inter : ∀ A ∈ F, ∀ B ∈ F, ¬ Disjoint A B)
    (h_not_star : ¬ IsStarFamily F) :
    ∃ x y z : α, x ≠ y ∧ y ≠ z ∧ x ≠ z ∧
      F = ({{x, y}, {y, z}, {x, z}} : Finset (Finset α)) := by
  have hF_nonempty : F.Nonempty := by
    ...
    simp [h]
  obtain ⟨A, hAF⟩ := hF_nonempty
  obtain ⟨x, y, hxy, rfl⟩ := card_eq_two.mp (hF_two A hAF)
  have hx_not_all : ¬ ∀ D ∈ F, x ∈ D := fun h => h_not_star ⟨x, h⟩
  push Not at hx_not_all
  obtain ⟨B, hBF, hxB⟩ := hx_not_all
  have hyB : y ∈ B := by
    obtain ⟨q, hqA, hqB⟩ := Finset.not_disjoint_iff.mp (h_inter {x, y} hAF B hBF)
    simp only [mem_insert, mem_singleton] at hqA
    rcases hqA with rfl | rfl
    · exact (hxB hqB).elim
    · exact hqB
  obtain ⟨z, hyz, hB⟩ := exists_pair_eq_of_mem_of_card_eq_two hyB (hF_two B hBF)
  have hxz : x ≠ z := by
    intro hxz
    apply hxB
    rw [hB, hxz]
    simp
  have hy_not_all : ¬ ∀ D ∈ F, y ∈ D := fun h => h_not_star ⟨y, h⟩
  push Not at hy_not_all
  obtain ⟨C, hCF, hyC⟩ := hy_not_all
  have hxC : x ∈ C := by
    obtain ⟨q, hqA, hqC⟩ := Finset.not_disjoint_iff.mp (h_inter {x, y} hAF C hCF)
    simp only [mem_insert, mem_singleton] at hqA
    rcases hqA with rfl | rfl
    · exact hqC
    · exact (hyC hqC).elim
  have hzC : z ∈ C := by
    obtain ⟨q, hqB, hqC⟩ := Finset.not_disjoint_iff.mp (h_inter B hBF C hCF)
    rw [hB] at hqB
    simp only [mem_insert, mem_singleton] at hqB
    rcases hqB with rfl | rfl
    · exact (hyC hqC).elim
    · exact hqC
  have hC : C = {x, z} :=
    eq_pair_of_card_eq_two_of_mem (hF_two C hCF) hxz hxC hzC
  refine ⟨x, y, z, hxy, hyz, hxz, Finset.Subset.antisymm ?_ ?_⟩
  · intro D hDF
    have hDA := mem_or_mem_of_not_disjoint_pair (h_inter D hDF {x, y} hAF)
    have hDBraw := h_inter D hDF B hBF
    rw [hB] at hDBraw
    have hDB := mem_or_mem_of_not_disjoint_pair hDBraw
    have hDCraw := h_inter D hDF C hCF
    rw [hC] at hDCraw
    have hDC := mem_or_mem_of_not_disjoint_pair hDCraw
    rcases hDA with hxD | hyD
    · rcases hDB with hyD | hzD
      · have hD := eq_pair_of_card_eq_two_of_mem (hF_two D hDF) hxy hxD hyD
        simp [hD]
      · have hD := eq_pair_of_card_eq_two_of_mem (hF_two D hDF) hxz hxD hzD
        simp [hD]
    · rcases hDC with hxD | hzD
      · have hD := eq_pair_of_card_eq_two_of_mem (hF_two D hDF) hxy hxD hyD
        simp [hD]
      · have hD := eq_pair_of_card_eq_two_of_mem (hF_two D hDF) hyz hyD hzD
        simp [hD]
  · intro D hD
...
theorem two_uniform_intersecting_not_star_card_eq_three
    ...
    (h_not_star : ¬ IsStarFamily F) :
    F.card = 3 := by
  obtain ⟨x, y, z, hxy, hyz, hxz, hF⟩ :=
    two_uniform_intersecting_not_star_eq_triangle F hF_two h_inter h_not_star
  rw [hF]
  have hAB : ({x, y} : Finset α) ≠ {y, z} := by
    intro h
    have hxmem : x ∈ ({y, z} : Finset α) := by rw [← h]; simp
    simp [hxy, hxz] at hxmem
  have hAC : ({x, y} : Finset α) ≠ {x, z} := by
    intro h
    have hymem : y ∈ ({x, z} : Finset α) := by rw [← h]; simp
    simp [hxy.symm, hyz] at hymem
  have hBC : ({y, z} : Finset α) ≠ {x, z} := by
    intro h
    have hymem : y ∈ ({x, z} : Finset α) := by rw [← h]; simp
    simp [hxy.symm, hyz] at hymem
  simp [hAB, hAC, hBC]

/-- **Hilton--Milner for `k = 2`.** On an `n`-element universe with `4 < n`, every
pairwise-intersecting family of two-element sets with no common element has cardinality at
most `hiltonMilnerBound n 2 = 3`. -/
theorem hilton_milner_stability_two {n : ℕ}
