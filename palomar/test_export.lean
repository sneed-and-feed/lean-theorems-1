...
def main : IO Unit := do
  ...
  IO.println "Testing export on Challenge..."
  let envC ← importModules #[{ module := `Challenge }] {}
  let decls : List Name := [
    `Quot, `Quot.mk, `Quot.lift, `Quot.ind,
    `TverbergsTheorem.tverbergs_theorem,
    `TverbergsTheorem.tverberg_1d,
    `TverbergsTheorem.colorful_tverberg_1d,
  ...
  IO.println "Running M.run on Challenge..."
  M.run envC do
    let _ ← initState envC []
    dumpMetadata
    for c in decls do
      IO.println s!"Dumping {c} from Challenge..."
      modify (fun st => { st with noMDataExprs := {} })
      dumpConstant c
  IO.println "Challenge export succeeded!"

  IO.println "Testing export on Solution..."
  let envS ← importModules #[{ module := `Solution }] {}
  IO.println "Running M.run on Solution..."
  M.run envS do
    let _ ← initState envS []
    dumpMetadata
    for c in decls do
      IO.println s!"Dumping {c} from Solution..."
      modify (fun st => { st with noMDataExprs := {} })
      dumpConstant c
  IO.println "Solution export succeeded!"
